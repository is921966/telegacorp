...
export function useRealtimeUpdates() {
  ...
  useEffect(() => {
    ...
    const trySubscribe = async () => {
      ...
      const { getExistingClient } = await import("../telegram/client");
      ...
      try {
        const { initPrefetchManager } = await import("../adapters/prefetch-manager");
        initPrefetchManager(client);
      } catch { /* prefetch adapter not configured — skip */ }
    };
