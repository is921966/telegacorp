...
import { getPlatformConfig, getPlatformStorage } from "../store/platform";

let clientInstance: TelegramClient | null = null;
let connectPromise: Promise<TelegramClient> | null = null;

/**
 * Clear stale GramJS TL schema cache from storage.
...
function clearStaleApiCache(): void {
  try {
    getPlatformStorage().removeItem("GramJs:apiCache");
  } catch {
...
export function getTelegramClient(sessionString = ""): TelegramClient {
  ...
  clientInstance = new TelegramClient(session, config.telegramApiId, config.telegramApiHash, {
    connectionRetries: 5,
    useWSS: true,
