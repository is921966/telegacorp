...
/** Platform-specific theme change callback (web: toggle DOM classes, mobile: context) */
let _onThemeChange: ((theme: "light" | "dark") => void) | null = null;

export function setThemeChangeCallback(cb: (theme: "light" | "dark") => void): void {
  _onThemeChange = cb;
}
...
export const useUIStore = create<UIStore>((set) => ({
  ...
  selectChat: (chatId) => {
    if (chatId) {
      import("../adapters/chat-priority").then(({ recordChatOpen }) => {
  ...
  setTheme: (theme) => {
    // Platform-specific theme application is handled via onThemeChange callback
